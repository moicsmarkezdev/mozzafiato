...
class RecentProductsSection extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.storage = JSON.parse(localStorage.getItem(theme.local_storage.recent_products));
      if (this.storage) {
        this.load();
      } else {
    ...
    async renderProductItem(t) {
      var e;
      var r;
      var s = await fetch(theme.urls.root + `/products/${t}?view=ajax-item`);
      if (s.ok) {
        e = await s.text();
        r = theme.utils.parseHtml(e, ".product--root");
